...
function getSharedWordSegmenter() {
	if (sharedWordSegmenter === null) sharedWordSegmenter = new Intl.Segmenter(segmenterLocale, { granularity: "word" });
...
function getSegmentMetrics(seg, cache) {
	...
	if (metrics === void 0) {
		metrics = {
			width: getMeasureContext().measureText(seg).width,
...
function clearMeasurementCaches() {
	...
	sharedGraphemeSegmenter$1 = null;
}
//#endregion
...
function findChunkIndexForStart(prepared, segmentIndex) {
	...
	while (lo < hi) {
		...
		if (segmentIndex < prepared.chunks[mid].consumedEndSegmentIndex) hi = mid;
		else lo = mid + 1;
	}
...
function normalizeLineStartWithChunk(prepared, start) {
	...
	if (graphemeIndex > 0) return {
		cursor: start,
		chunkIndex
	};
	const chunk = prepared.chunks[chunkIndex];
	if (chunk.startSegmentIndex === chunk.endSegmentIndex && segmentIndex === chunk.startSegmentIndex) return {
		cursor: {
			segmentIndex,
			graphemeIndex: 0
		},
	...
	while (segmentIndex < chunk.endSegmentIndex) {
		const kind = prepared.kinds[segmentIndex];
		if (kind !== "space" && kind !== "zero-width-break" && kind !== "soft-hyphen") return {
			cursor: {
				segmentIndex,
				graphemeIndex: 0
			},
...
function countPreparedLines(prepared, maxWidth) {
	if (prepared.simpleLineWalkFastPath) return countPreparedLinesSimple(prepared, maxWidth);
...
function layoutNextLineRange(prepared, start, maxWidth) {
	...
	if (prepared.simpleLineWalkFastPath) return layoutNextLineRangeSimple(prepared, normalized.cursor, maxWidth);
	const chunk = prepared.chunks[normalized.chunkIndex];
	if (chunk.startSegmentIndex === chunk.endSegmentIndex) return {
		startSegmentIndex: chunk.startSegmentIndex,
		startGraphemeIndex: 0,
		endSegmentIndex: chunk.consumedEndSegmentIndex,
		endGraphemeIndex: 0,
	...
	function appendBreakableSegmentFrom(segmentIndex, startGraphemeIndex) {
		...
		for (let g = startGraphemeIndex; g < gWidths.length; g++) {
			...
			if (lineW + gw > maxWidth + lineFitEpsilon) return finishLine();
			lineW += gw;
			lineEndSegmentIndex = segmentIndex;
			lineEndGraphemeIndex = g + 1;
		}
	...
	function maybeFinishAtSoftHyphen(segmentIndex) {
		...
		if (gWidths !== null) {
			...
			if (fitCount > 0) return finishLine(segmentIndex, fitCount, fittedWidth + discretionaryHyphenWidth);
		}
		if (pendingBreakFitWidth <= maxWidth + lineFitEpsilon) return finishLine(pendingBreakSegmentIndex, 0, pendingBreakPaintWidth);
		return null;
	}
	for (let i = normalized.cursor.segmentIndex; i < chunk.endSegmentIndex; i++) {
		...
		if (lineW + w > maxWidth + lineFitEpsilon) {
			const currentBreakFitWidth = lineW + (kind === "tab" ? 0 : lineEndFitAdvances[i]);
			const currentBreakPaintWidth = lineW + (kind === "tab" ? w : lineEndPaintAdvances[i]);
			if (pendingBreakKind === "soft-hyphen" && engineProfile.preferEarlySoftHyphenBreak && pendingBreakFitWidth <= maxWidth + lineFitEpsilon) return finishLine(pendingBreakSegmentIndex, 0, pendingBreakPaintWidth);
			...
			if (pendingBreakSegmentIndex >= 0 && pendingBreakFitWidth <= maxWidth + lineFitEpsilon) {
				if (lineEndSegmentIndex > pendingBreakSegmentIndex || lineEndSegmentIndex === pendingBreakSegmentIndex && lineEndGraphemeIndex > 0) return finishLine();
	...
	if (pendingBreakSegmentIndex === chunk.consumedEndSegmentIndex && lineEndGraphemeIndex === 0) return finishLine(chunk.consumedEndSegmentIndex, 0, pendingBreakPaintWidth);
...
function profilePrepare(text, font, options) {
	...
	for (const widths of prepared.breakableWidths) if (widths !== null) breakableSegments++;
	return {
		analysisMs: t1 - t0,
		measureMs: t2 - t1,
		totalMs: t2 - t0,
...
//#endregion
export { clearCache, layout, layoutNextLine, layoutWithLines, prepare, prepareWithSegments, profilePrepare, setLocale, walkLineRanges };
